...
-- MIN(숫자형컬럼), MAX()
SELECT max(first_name), min(salary), min(salary)
FROM employees;
...
-- department_id가 30에서 90 사이이고, 부서직군별급여총액이 20000달러 이상인 데이터만 보일것
SELECT department_id, job_id, sum(salary) AS 부서직군별급여총액, count(*)
FROM employees
...
-- order by에는 컬럼의 순번(1번 시작)으로 컬럼명을 대체 가능
SELECT department_id, job_id, sum(salary) AS 부서직군별급여총액, count(*)
FROM employees
WHERE department_id BETWEEN 30 AND 90
GROUP BY department_id, job_id
...
-- RANK() 등수 공동등수 번호 띄우기, DENSE_RANK() 등수 번호가 순차적으로 올라감
-- ROW_NUMBER() 현재 데이터 행번호 출력
SELECT EMPLOYEE_ID, last_name, salary
	 , rank() OVER (ORDER BY  salary desc) AS "랭크"
	 , dense_rank() OVER (ORDER BY  salary desc) AS "댄스랭크" -- 일반적으로 많이 사용
