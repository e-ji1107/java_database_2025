...
class MainWindow(QMainWindow):
    ...
    def btnModClick(self):
        ...
        if std_id == '' or std_name == '' or std_regyear == '':
            QMessageBox.warning(self, '경고', '학생번호, 학생이름, 입학년도 필수 입력')
            return # 함수 호출
        else:
            print('DB 수정 진행')
            values = (std_name, std_mobile, std_regyear, std_id)
            self.modeData(values)

            if self.modeData(values) == True:
                QMessageBox.about(self, '성공', '학생정보 수정 성공!')
            else:
                QMessageBox.warning(self, '실패', '관리자에게 문의하세요!')

            self.loadData()
            self.clearInputs() # 인풋값 삭제 함수 호출
    ...
    def btnDelClick(self):
        ...
        std_id = self.input_std_id.text()
        if std_id == '':
            QMessageBox.warning(self, '경고', '학생번호 필수 입력')
            return # 함수 호출
        else:
            print('DB 삭제 진행')
            values = (int(std_id),) # Oracle은 파라미터 타입에 민감. 정확한 타입을 사용해야함
            

        if self.delData(values) == True:
            QMessageBox.about(self, '성공', '학생정보 삭제 성공!')
        else:
            QMessageBox.warning(self, '실패', '삭제 실패 관리자에게 문의하세요!')
